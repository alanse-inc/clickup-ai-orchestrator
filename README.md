...
> **推奨**: ターゲットリポジトリに `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY`（オーケストレーターと同じ GitHub App）を設定すると、Claude Code が作成した PR で CI が自動トリガーされます。Organization Secret として設定すれば全リポジトリで共有できます。詳細はセットアップガイドを参照してください。
