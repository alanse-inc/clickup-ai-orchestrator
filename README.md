...
> **推奨**: `CI_APP_ID` / `CI_APP_PRIVATE_KEY`（オーケストレーターと同じ GitHub App の値）を設定すると、Claude Code が作成した PR で CI が自動トリガーされます。Organization Secret として設定する場合は組織名を含む名前に変更してください。詳細はセットアップガイドを参照してください。
